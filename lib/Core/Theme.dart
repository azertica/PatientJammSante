import 'package:flutter/material.dart';

class ThemeColor {
  //Background color
  final primaryBgColor = const Color.fromARGB(255, 15, 23, 42);
  final secondaryBgColor = const Color.fromARGB(255, 17, 162, 233);
  final thirdBgColor = const Color.fromARGB(255, 30, 41, 58);
  
  //Color
  final primaryColor = const Color.fromARGB(255, 244, 249, 253);
  final secondaryColor = const Color.fromARGB(255, 148, 163, 187);
  final thirdColor = const Color.fromARGB(255, 34, 157, 182);
}